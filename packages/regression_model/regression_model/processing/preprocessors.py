...
class RareLabelCategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y = None):
        ...
        for var in self.variables:
            # The enconder will learn the most frequent categories
            t_temp = pd.Series(X[var].value_counts() / np.float(len(X)))

            # Keep the frequent labels.
            self.encoder_dict_[var] = list(t_temp[t_temp >= self.tol].index)
...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # check if transformer introduces null values.
